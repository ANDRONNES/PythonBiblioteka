        ...
        if not isBookExists(Id_ksiazka):
            ...
            if not isReaderExists(Id_czytelnik):
                ...
                if Data_rozpoczecnia_rezerwacji >= Data_zakonczenia_rezerwacji:
                    ...
                    VALUES(?,?,?,?)''', (
                        Id_ksiazka, Id_czytelnik, Data_rozpoczecnia_rezerwacji, Data_zakonczenia_rezerwacji))
                    conn.commit()
                ...
                case 1:
                    ...
                    try:
                        if not isBookExists(newBook_id):
                            ...
                            cursor.execute('SELECT Ksiazka_id_ksiazki FROM Rezerwacja Where id_rezerwacji = ? ',
                                           (id_rezerwacji,))
                            oldBook = cursor.fetchone()[0]
                            cursor.execute('Update Ksiazka set Status_id_statusu = 1 where id_ksiazki = ?', (oldBook,))
                            cursor.execute('Select Czytelnik_id_czytelnika from Rezerwacja Where id_rezerwacji = ?',
                                           (id_rezerwacji,))
                            czytelnik = cursor.fetchone()[0]
                            cursor.execute('''Insert INTO Historia(Czytelnik_id_czytelnika,Ksiazka_id_ksiazki,opis_operacji,data)
                                                                        Values(?,?,?,?)''',
                                           (czytelnik, newBook_id, "Wymiana rezerwowanej książki", date.today()))
                            ...
                            if cursor.fetchone()[0] == 1:
                                ...
                                cursor.execute('Update Ksiazka set Status_id_statusu = 3 where id_ksiazki = ?',
                                               (newBook_id,))
                    ...
                    try:
                        ...
                        if (newCzytelnik_id == cursor.fetchone()[0]):
                            ...
                            if not isReaderExists(newCzytelnik_id):
                                ...
                                cursor.execute('Select Ksiazka_id_ksiazki from rezerwacja Where id_wypozyczenia = ?',
                                               (id_rezerwacji,))
                                bookId = cursor.fetchone()[0]
                                cursor.execute('''Insert INTO Historia(Czytelnik_id_czytelnika,Ksiazka_id_ksiazki,opis_operacji,data)
                                                                                                            Values(?,?,?,?)''',
                                               (newCzytelnik_id, bookId,
                                                "Rezerwacja (zmiana czytelnika w rezerwacji)", date.today()))
                                conn.commit()
                ...
                case 3:
                    ...
                    try:
                        ...
                        if Data_Rezerwacji >= Data_Zwrotu:
                            ...
                            cursor.execute('''INSERT INTO Historia(Czytelnik_id_czytelnika,Ksiazka_id_ksiazki,opis_operacji,data)
                                                                          VALUES (?,?,?,?)''',
                                           (values[0], values[1], "Przeniesienie rozpoczęcia rezerwacji",
                                            date.today()))

                            cursor.execute(
                                'UPDATE Rezerwacja SET Data_rozpoczecia = ? Where id_rezerwacji = ?',
                                (newData_Rezerwacji, id_rezerwacji,))
                            conn.commit()
                ...
                case 4:
                    ...
                    try:
                        ...
                        if Data_Zwrotu <= Data_Wypozyczenia:
                            ...
                            cursor.execute('''INSERT INTO Historia(Czytelnik_id_czytelnika,Ksiazka_id_ksiazki,opis_operacji,data)
                                                                                                          VALUES (?,?,?,?)''',
                                           (values[0], values[1], "Przeniesienie zakończenia rezerwacji",
                                            date.today()))
                            cursor.execute(
                                'UPDATE Rezerwacja SET Data_rozpoczecia = ? Where id_rezerwacji = ?',
